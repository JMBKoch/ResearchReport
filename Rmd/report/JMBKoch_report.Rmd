...
We focus^[We also computed the Mean Squared Error and Relative Bias of the posterior mean estimates. Moreover all outcomes were also calculated based on median posterior estimates of the model parameters. No findings that counter the conclusions from the Mean Absolute Bias emerged. Plots summarizing the alternative outcomes can be found on https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/plots.] on the Mean (Absolute) Bias of the posterior mean estimates of all model parameters, per set of conditions ($\bar{\theta} | conditions$). Hence, for every model parameter $\theta$ and for every set of conditions that has been sampled from for $N_{rep}$ replications:
