...
We focus\footnote{We also computed the Mean Squared Error and Relative Bias of the posterior mean estimates. Moreover all outcomes were also calculated based on median posterior estimates of the model parameters. No findings that counter the conclusions from the Mean Absolute Bias emerged. Plots summarizing the alternative outcomes can be found on \url{https://github.com/JMBKoch/1vs2StepBayesianRegSEM/tree/main/Rmd/plots}.} on the Mean (Absolute) Bias of the posterior mean estimates of all model parameters, per set of conditions (\(\bar{\theta} | conditions\)). Hence, for every model parameter \(\theta\) and for every set of conditions that has been sampled from for \(N_{rep}\) replications:
...
Of course, the large amount of bias in the SVNP is not surprising, as it is clearly noted that the method requires a 2-step approach to avoid bias. However, this approach heavily depends on a successful selection of non-zero cross-loadings. Muthen and Asparouhov (2012) advise a power (true positive rate) in selecting non-zero cross-loadings of at least .80. However, only under a single set of conditions (N = 200, sigma = 0.01, cross-loading = 0.5) this power was reached in our study, which suggests that even a 2-step approach would in practice be no ideal solution. This serves to illustrate the need for more advanced priors such as the RHSP, although different selection rules (see Zhang, Pan, \& Ip, 2021) may show a better performance than the 95\% credible intervals suggested by Muthen and Asparouhov (2012).
