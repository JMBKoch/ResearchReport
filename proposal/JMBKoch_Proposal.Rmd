...
A common model-constraint in *Confirmatory Factor Analysis (CFA)* is
...
[@jacobucci_regularized_2016; @muthen_bayesian_2012].

As solution, @muthen_bayesian_2012 proposed that rather than fixing all
...
@lu_bayesian_2016 pointed out that the Bayesian CFA approach by
...
alternative priors. Here, the *Regularized Horseshoe Prior*
...
$\lambda_j = 0$ in Figure 1), and a *local* component $\tau_j$, which
...
sampling of the model [@ghosh_use_2018; @piironen_sparsity_2017]. To
...
using the No-U-Turn-sampler [@homan_no-u-turn_2014; @betancourt_conceptual_2018], where each time
